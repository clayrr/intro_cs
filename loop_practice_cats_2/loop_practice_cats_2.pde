...
void setup() {
 ...
 void draw(){
  x = 300;
  //if using draw then use framecount
  for(x = 0; x <= 1400; x += 150){
    //can use mod with x
    //if (x % 2 = 0), person, else, kitty
    if (isCat){
      
      if (frameCount % 50 == 0) {  
      tower(x, 300, 200, "kitty", color(222, 135, 12));
      }
      isCat = false;
    }
    else{
      if (frameCount % 50 == 0) {  
      tower(x, 300, 200, "person", color(222, 135, 12));
      }
      isCat = true;
    }
  }
